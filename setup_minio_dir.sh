...
# mcli mb tidak akan error jika direktori sudah ada, jadi ini aman dijalankan ulang
create_dir_if_not_exists() {
    local full_path="$1"
    echo "Creating MinIO directory (object prefix): ${full_path} ..."
    mcli mb "${full_path}"
    # Anda bisa menambahkan pengecekan error jika mcli mb gagal untuk alasan selain sudah ada
    # if [ $? -ne 0 ]; then
...
# 1. Pastikan bucket utama ada (mcli mb juga bisa membuat bucket)
...
mcli tree "${MINIO_ALIAS}/${MAIN_BUCKET}"